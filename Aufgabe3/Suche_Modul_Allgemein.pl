...
% Falls der Kindzustand bereits im Pfad vorhanden war, wird der gesamte Pfad verworfen, denn er würde nur in einem Zyklus enden. 
%(Dies betrifft nicht die Fortsetzung des Pfades mit Geschwister-Kindern.) 
%Es wird nicht überprüft, ob der Kindzustand in einem anderen Pfad vorkommt, denn möglicherweise ist dieser Weg der günstigere.
...
%hiermit werden alle Zustände und ggf. Heuristiken ausgegeben
%write_actions(Solve):-
%ansi_format([bold,fg(red)], "Lösung: ~w",[Solve]),nl.
